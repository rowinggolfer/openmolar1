...
class chartWidget(QtWidgets.QWidget):
    ...
    def multiSelectADD(self):
        ...
        if self.selected in self.multiSelection:
            ...
            return False
        if self.selected not in self.multiSelection:
        ...
        if self.props != []:
            ...
            for prop in self.props:
                ...
                if prop == "pv" and self.isUpper:
                    poly = QtGui.QPolygon()
                    poly.setPoints([self.rect.topLeft().x(),
                                    self.rect.topLeft().y(),
                    ...
                    self.painter.drawText(
                        self.rect.adjusted(0, 0, 0, -self.rect.height() / 2),
                        QtCore.Qt.AlignCenter,
                        prop)
                ...
                if prop == "pv" and not self.isUpper:
                    poly = QtGui.QPolygon()
                    poly.setPoints([self.rect.bottomLeft().x(),
                                    self.rect.bottomLeft().y(),
                                    self.rect.bottomRight().x(),
                                    self.rect.bottomRight().y(),
                    ...
                    self.painter.drawPolygon(poly)
                    self.painter.drawText(
                        self.rect.adjusted(0, self.rect.height() / 2, 0, 0),
                ...
                if self.backTooth:
                    if "fs" in prop:
                        poly = QtGui.QPolygon()
                        poly.setPoints([dx, ey - 1, fx, ey - 1, fx + 1,
                                        ey + 1, dx, ey + 1])
                        shapes.append(poly)
                        poly = QtGui.QPolygon()
                        poly.setPoints([ex - 1, dy, ex + 1, dy, ex + 1,
                                        fy, ex - 1, fy])
                        shapes.append(poly)
                    elif "dr" in prop:
                        poly = QtGui.QPolygon()
                        ...
                        poly = QtGui.QPolygon()
                        poly.setPoints([dx, dy, fx, dy, fx, fy, dx, fy])
                        shapes.append(poly)
                    elif "m" in prop:
                        poly = QtGui.QPolygon()
                        poly.setPoints([gx, dy, ix, by, ix, hy, gx, fy])
                        shapes.append(poly)
                    elif "d" in prop:
                        poly = QtGui.QPolygon()
                        poly.setPoints([ax, by, cx, dy, cx, fy, ax, hy])
                        shapes.append(poly)
                    elif "p" in prop:
                        poly = QtGui.QPolygon()
                        poly.setPoints([bx, iy, dx, gy, fx, gy, hx, iy])
                        shapes.append(poly)
                    elif "b" in prop:
                        poly = QtGui.QPolygon()
                        poly.setPoints([bx, ay, hx, ay, fx, cy, dx, cy])
                        shapes.append(poly)
                else:  # front tooth
                    if "dr" in prop:
                        poly = QtGui.QPolygon()
...
if __name__ == "__main__":
    ...
    for properties in (
            ...
            ("ur4", "mop,co"), ("ur3", "AT"), ("ur2", "di,co b"),
            ("ur1", "pv rt"), ("ul1", "cr,pj"), ("ul2", "dp,co b,co"),
            ("ul3", "di p,go"), ("ll3", "pv"), ("ll2", "dip"), ("ll1", "midi"),
            ("lr1", "mpd"), ("lr2", "mld,gl"), ("lr3", "dr"),
            ("ul4", "do"), ("ul6", "mo"), ("lr8", "("), ("ul7", "mop,co"),
            ("ur8", "mdb,gl mpd,go ob"), ("ll4", "b,gl dl,co"),
            ("ll5", "ob ml"), ("ll6", "mod,co"), ("ll7", "pe"),
            ("ll8", "ue !watch"), ("lr4", "im/tit"), ("lr5", "dr"),
            ("lr7", "fs"), ("lr6", "modbl,pr")):

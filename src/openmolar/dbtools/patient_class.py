...
class patient(object):
    ...
    category="", type_="", descr=None, fee=None, ptfee=None,
    completed=False, feescale=None, carriedover=False):
        '''
        add an item to the patient's estimate
        '''
        LOGGER.debug(
        "Patient.addToEstimate %s %s %s %s %s %s %s %s %s %s %s %s"% (
            number, itemcode, dent, 
            csetype, category, type_, descr, fee, ptfee,
            completed, feescale, carriedover)
            )
        ...
        if category != "":
            ...
            cat_ = table.getTxCategory(itemcode)

        est.tx_hashes = [str(hash("%s %s %s"% (cat_, existing_no+1, type_)))]
    ...
    def tx_hashes(self):
        ...
        for att in CURRTRT_ROOT_ATTS:
            treats = self.__dict__[att+"cmp"] + " " + self.__dict__[att+"pl"] 

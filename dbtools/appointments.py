...
def getWorkingDents(adate,dents=()):
    '''dentists are part time, or take holidays...
    this proc takes a date, and optionally a tuple of dents
    ...
    #   starts.append(row[1])
    #    ends.append(row[2])
    #return (tuple(dents),tuple(starts),tuple(ends))                                                        #return arg like (4,5)
...
def printableDaylistData(adate,dent):                                                    #removed dent=4 on 23/4/2009 - assumed uneccessary
    ...
    if daydata!=():                                                             #dentist is working!!
        ...
        #cursor.execute('SELECT start,end,name,serialno,code0,code1,code2,note FROM aslot WHERE %s \
        #ORDER BY start'%query)                                                     #SQL QUERY
        cursor.execute('SELECT start,end,concat(patients.title," ",patients.fname," ",patients.sname),\
        patients.serialno,code0,code1,code2,note,name FROM patients right join aslot on patients.serialno=aslot.serialno\
        where %s order by start'%query)
...
def daysummary(adate,dent):                                                    #removed dent=4 on 23/4/2009 - assumed uneccessary
    ...
    cursor.execute('''SELECT start,end FROM aday WHERE \
    adate="%s" and (flag=1 or flag=2) %s'''%(adate,apptix))                    #SQL QUERY to get start and finish
    daydata= cursor.fetchall()
    query=""
    retarg=()                                                                 #now get data for those days so that we can find slots within
    if daydata!=():
        query= ' adate = "%s" and apptix = %d '%(adate,dent)
        cursor.execute('SELECT start,end FROM aslot WHERE %s \
        AND flag0!=-128 ORDER BY start'%query)                                 #SQL QUERY
...
def getBlocks(adate,dent):
    ...
    if dent==-1:                                                               ##todo - is this used?
        ...
        apptix="and apptix=%d"%dent
    cursor.execute('''SELECT start,end FROM aday WHERE \
    adate="%s" AND (flag=1 OR flag=2) %s'''%(adate,apptix))                    #SQL query to get start and end
    daydata= cursor.fetchall()                                                 #now get data for those days so that we can find slots within

    query=""
    if daydata!=():
        query= ' adate = "%s" and apptix = %d '%(adate,dent)
        cursor.execute('SELECT start,end FROM aslot WHERE \
...
def clearEms(adate):
    ...
    cursor=db.cursor()
    result=False
    if cursor.execute('delete from aslot WHERE adate="%s" and flag0=-128 and name!="LUNCH"'%adate):
        db.commit()                              #SQL QUERY
        result=True
    cursor.close()
    #db.close()
...
def get_pts_appts(sno):
    '''gets appointments from the apr table which stores \
    appointments from patients perspective (including appts \
...
def add_pt_appt(serialno,practix,length,code0,aprix=-1,\
    code1="",code2="",note="",datespec="",flag1=80,\
    ...
    if aprix==-1:                                                              #this means put the appointment at the end
        cursor.execute('SELECT aprix FROM apr WHERE \
        serialno=%d ORDER BY aprix'%serialno)                                  #SQL QUERY to get the aprix _KEY VALUE_
        data= cursor.fetchall()
        if data!=():
            aprix=data[-1][0]+1
        else:
            aprix=1
    columns='serialno,aprix,practix,code0,code1,code2,\
    note,length,flag0,flag1,flag2, flag3,flag4,datespec'
    values='%d,%d,%d,"%s","%s","%s","%s",%d,%d,%d,%d,\
    %d,%d,"%s"'%(serialno,aprix,practix,code0,code1,\
    code2,note,length,flag0,flag1,flag2,flag3,flag4,datespec)
    if cursor.execute('INSERT INTO apr (%s) VALUES (%s)'\
    %(columns,values)):                                                        #SQL QUERY
        db.commit()
        result=True
    else:
...
def pt_appt_made(serialno,aprix,date,time,dent):
    '''modifies the apr table, finding the unscheduled version\
    and putting scheduled data in'''
    db = connect()
    cursor = db.cursor()
    if cursor.execute('UPDATE apr SET adate="%s" , \
    atime=%d, practix=%d WHERE serialno=%d AND \
    aprix=%d'%(date,time,dent,serialno,aprix)):                                #SQL QUERY
        db.commit()
        result=True
    else:
        result=False
...
def made_appt_to_proposed(serialno,aprix):
    ...
    db = connect()
    cursor = db.cursor()
    if cursor.execute('UPDATE apr SET adate=NULL , atime=NULL \
    WHERE serialno=%d AND aprix=%d'%(serialno,aprix)):                         #SQL QUERY
        db.commit()
        result=True
    else:
        result=False
...
def make_appt(adate,apptix,start,end,name,serialno,\
    ...
    db = connect()
    cursor = db.cursor()
    columns='adate,apptix,start,end,name,serialno,code0,\
    code1,code2,note,flag0,flag1,flag2,flag3'
    values='"%s",%d,%d,%d,"%s",%d,"%s","%s","%s","%s",\
    %d,%d,%d,%d'%(adate,apptix,start,end,name,serialno,\
    code0,code1,code2,note,flag0,flag1,flag2,flag3)
...
def delete_appt_from_apr(serialno,aprix):
    ...
    try:
        db = connect()
        cursor = db.cursor()
        cursor.execute('SELECT atime,adate,practix,code0 FROM apr \
        WHERE serialno=%d AND aprix=%d'%(serialno,aprix))                      #SQL QUERY - get data from the table
        val=cursor.fetchall()                                                  #safer to do this than assume the patient record is correct??
        if val!=():                                                            #multi-user wise
            cursor.execute('DELETE FROM apr WHERE serialno=\
            %d AND aprix=%d'%(serialno,aprix))
        else:
            return False
        ...
        for curval in curvals:
            i+=1
            cursor.execute('UPDATE apr set aprix=%d where \
...
def delete_appt_from_aslot(dent,start,adate,serialno):                         #delete from the appointment book proper
        query= 'DELETE FROM aslot WHERE serialno=%d AND apptix=%d AND \
        start=%d AND adate="%s"'%(serialno,dent,start,adate)
        result=False
        db=connect()
        cursor=db.cursor()
...
def daysSlots(adate,dent):                                                     #was dent="NW" 23/2/2009
    ...
    cursor.execute('''SELECT start,end FROM aday WHERE adate="%s" and \
    (flag=1 or flag=2) AND maxtime>0 %s'''%(adate,apptix))                     #SQL QUERY
    daydata= cursor.fetchall()                                                 #now get data for those days so that we can find slots within
    query=""
    if daydata!=():
        query= ' adate = "%s" and apptix = %d '%(adate,\
        localsettings.apptix[dent])
        cursor.execute('SELECT start,end FROM aslot WHERE %s ORDER BY start'\
        %query)                                                                #SQL QUERY
        results=cursor.fetchall()
        cursor.close
        #db.close()
...
def future_slots(length,startdate,enddate,dents,override_emergencies=False):
    '''get a list of possible appointment positions (between
    startdate and enddate) that can be offered to the
    ...
    if dents!=():
        ...
        mystr=""
    cursor.execute('''SELECT adate,apptix,start,end FROM
    aday WHERE adate>="%s" AND adate<="%s" AND (flag
    =1 OR flag= 2) AND maxtime>=%d %s ORDER BY adate'''
    %(startdate,enddate,length,mystr))                                         #SQL QUERY
    ...
    for day in possible_days:
        ...
        cursor.execute('select start,end from aslot where %s \
        and flag0!=72 order by start'%query)                                   #flag0!=72 necessary to avoid zero length apps like pain/double/fam
        results=cursor.fetchall()
        daystart=day[2]
        dayfin=day[3]

...
Ui_phraseBook,Ui_changeDatabase,Ui_related_patients,Ui_raiseCharge,Ui_options,Ui_surgeryNumber,Ui_payments,paymentwidget,\
...
from openmolar.ptModules import patientDetails,notes,plan,referral,standardletter,debug_html,estimates                                                                                           
...
class feeClass():
    def raiseACharge(self):
        ...
        if Dialog.exec_():
            ...
            self.updateFees()
            self.pt.addHiddenNote("treatment"," %s - fee %.02f"%(str(dl.lineEdit.text().toAscii()),fee))
    ...
    def takePayment(self):
        if self.pt.serialno==0:
            self.advise("No patient Selected <br />Monies will be allocated to Other Payments, and no receipt offered",1)
        ...
        if dl.exec_():
            ...
            if cashbook.paymenttaken(paymentPt.serialno,name,paymentPt.dnt1,paymentPt.cset,cash,cheque,debit,credit,sundries,hdp,other):
                ...
                self.advise("error applying payment.... sorry!<br />Please write this down and tell Neil what happened",2)
    ...
    def loadFeesTable(self):
        ...
        fdKeys.sort()
        print fdKeys
        tables=(self.ui.fees_diagnosis_tableWidget,self.ui.fees_prevention_tableWidget,self.ui.fees_perio_tableWidget,
        self.ui.fees_conservation_tableWidget,self.ui.fees_surgical_tableWidget,self.ui.fees_prosthetics_tableWidget,self.ui.fees_ortho_tableWidget,
...
class appointmentClass():  
    ...
    def newAppt(self):
        ...
        if Dialog.exec_():
            ...
            if appointments.add_pt_appt(self.pt.serialno,practix,length,code0,-1,code1,code2,note,"",cst):                  
        ...
        if dateText=="TBA" or QtCore.QDate.fromString(dateText,"dd'/'MM'/'yyyy")<QtCore.QDate.currentDate():                                                                                  
            ...
            if result == QtGui.QMessageBox.Ok:
                ...
                if appointments.delete_appt_from_aslot(dent,start,adate,self.pt.serialno):
                    ...
                    result=QtGui.QMessageBox.question(self.mainWindow,"Question","Removed from appointment book - keep for rescheduling?",
    ...
    def modifyAppt(self):
        ...
        if rowno==-1:
            ...
            if Dialog.exec_():
                ...
                code2=dl.trt3_comboBox.currentText()
                note=str(dl.lineEdit.text().toAscii())
                start=localsettings.humanTimetoWystime(str(self.ui.ptAppointmentTableWidget.item(rowno,2).text()))
                aprix=int(self.ui.ptAppointmentTableWidget.item(rowno,9).text())
                ...
                if self.pt.cset=="":
                    ...
                    cst=ord(self.pt.cset[0])                                                                                                        
                appointments.modify_pt_appt(adate,aprix,self.pt.serialno,practix,length,code0,code1,code2,note,"",cst)                
                if dateText=="TBA":
                    if dl.makeNow:
                        self.makeApptButtonClicked()
                else:
                    if not appointments.modify_aslot_appt(adate,practix,start,self.pt.serialno,code0,code1,code2,note,cst,0,0,0):
    ...
    def makeAppt(self,arg):
        ...
        if selectedDent!=dentist: 
            ...
            result=QtGui.QMessageBox.question(self.mainWindow,"Confirm",message,QtGui.QMessageBox.Ok,QtGui.QMessageBox.Cancel)                                                                   
        ...
        if slotlength==length:   
            ...
            if appointments.make_appt(
                ...
                if appointments.pt_appt_made(self.pt.serialno,aprix,selecteddate.toPyDate(),selectedtime,localsettings.apptix[selectedDent]):   
            ...
            self.advise("error - the appointment doesn't fit there.. slotlength is %d and we need %d"%(slotlength,length),2)
    ...
    def receptionSummary(self,esthtml):                                                                    
        #--populate the receptionist (admin) summary page
        self.advise("filling reception tables")
        self.ui.moneytextBrowser.setText(esthtml)
        self.layout_apptTable()
    ...
    def aptOVviewMode(self,Viewmode=True):
        ...
        for cb in (self.ui.aptOV_apptscheckBox,self.ui.aptOV_emergencycheckBox,self.ui.aptOV_lunchcheckBox):
    ...
    def clearTodaysEmergencyTime(self):
        ...
        result=QtGui.QMessageBox.question(self.mainWindow,"Confirm","Clear today's emergency slots?",QtGui.QMessageBox.Yes,
        QtGui.QMessageBox.No)
        if result==QtGui.QMessageBox.Yes:
            self.advise("Cleared %d emergency slots"%appointments.clearEms(localsettings.sqlToday()),1)
    ...
    def apptOVdents(self):
        ...
        #--change their values                                                                      
        for dent in self.ui.aptOVdent_checkBoxes.keys():                                                     
            self.ui.aptOVdent_checkBoxes[dent].setCheckState(self.ui.aptOV_alldentscheckBox.checkState())
        #--reconnect
    ...
    def apptOVhygs(self):
        ...
        for dent in self.ui.aptOVhyg_checkBoxes.keys():                                                      
            self.ui.aptOVhyg_checkBoxes[dent].setCheckState(self.ui.aptOV_allhygscheckBox.checkState())
        self.connectAptOVhygcbs()                                                                            
        self.layout_apptOV()
    def findApptButtonClicked(self):
        r=self.ui.ptAppointmentTableWidget.currentRow()
        d=QtCore.QDate.fromString(self.ui.ptAppointmentTableWidget.item(r,0).text(),"dd'/'MM'/'yyyy")
        QtCore.QObject.disconnect(self.ui.main_tabWidget,QtCore.SIGNAL("currentChanged(int)"),self.handle_mainTab)
        self.ui.appointmentCalendarWidget.setSelectedDate(d)
        self.ui.main_tabWidget.setCurrentIndex(1)
        QtCore.QObject.connect(self.ui.main_tabWidget,QtCore.SIGNAL("currentChanged(int)"),self.handle_mainTab)
    def layout_apptOV(self):
        ...
        i=0
        for label in (self.ui.mondayLabel,self.ui.tuesdayLabel,self.ui.wednesdayLabel,self.ui.thursdayLabel,self.ui.fridayLabel):
...
class signals():
    def setupSignals(self):
        ...
        QtCore.QObject.connect(self.ui.medNotes_pushButton,QtCore.SIGNAL("clicked()"), self.showMedNotes)
        QtCore.QObject.connect(self.ui.callXrays_pushButton,QtCore.SIGNAL("clicked()"), self.callXrays)
        QtCore.QObject.connect(self.ui.phraseBook_pushButton,QtCore.SIGNAL("clicked()"), self.phraseBookDialog)

        #admin page
        QtCore.QObject.connect(self.ui.home_pushButton,QtCore.SIGNAL("clicked()"), self.home)
        QtCore.QObject.connect(self.ui.newPatientPushButton,QtCore.SIGNAL("clicked()"), self.enterNewPatient)
        QtCore.QObject.connect(self.ui.findButton,QtCore.SIGNAL("clicked()"), self.find_patient)
        QtCore.QObject.connect(self.ui.reloadButton, QtCore.SIGNAL("clicked()"), self.reload_patient)
        ...
        QtCore.QObject.connect(self.ui.relatedpts_pushButton, QtCore.SIGNAL("clicked()"), self.find_related)
        QtCore.QObject.connect(self.ui.daylistBox, QtCore.SIGNAL("currentIndexChanged(int)"), self.todays_pts)
        QtCore.QObject.connect(self.ui.ptAppointmentTableWidget,QtCore.SIGNAL("itemSelectionChanged()"), self.ptApptTableNav)
        QtCore.QObject.connect(self.ui.printAccount_pushButton,QtCore.SIGNAL("clicked()"), self.printaccount)
        QtCore.QObject.connect(self.ui.printEst_pushButton,QtCore.SIGNAL("clicked()"), self.printEstimate)

        QtCore.QObject.connect(self.ui.printRecall_pushButton,QtCore.SIGNAL("clicked()"), self.printrecall)
        QtCore.QObject.connect(self.ui.takePayment_pushButton,QtCore.SIGNAL("clicked()"), self.takePayment)

        #admin summary widgets
        QtCore.QObject.connect(self.ui.newAppt_pushButton,QtCore.SIGNAL("clicked()"), self.newAppt)
        QtCore.QObject.connect(self.ui.makeAppt_pushButton,QtCore.SIGNAL("clicked()"), self.makeApptButtonClicked)
        QtCore.QObject.connect(self.ui.clearAppt_pushButton,QtCore.SIGNAL("clicked()"), self.clearApptButtonClicked)
        QtCore.QObject.connect(self.ui.modifyAppt_pushButton,QtCore.SIGNAL("clicked()"), self.modifyAppt)
        QtCore.QObject.connect(self.ui.findAppt_pushButton,QtCore.SIGNAL("clicked()"), self.findApptButtonClicked)
        QtCore.QObject.connect(self.ui.printAppt_pushButton,QtCore.SIGNAL("clicked()"), self.printApptCard)
        QtCore.QObject.connect(self.ui.printGP17_pushButton,QtCore.SIGNAL("clicked()"), self.printGP17)

        #printing buttons
        QtCore.QObject.connect(self.ui.receiptPrintButton,QtCore.SIGNAL("clicked()"), self.printDupReceipt)
        QtCore.QObject.connect(self.ui.exportChartPrintButton,QtCore.SIGNAL("clicked()"), self.printChart)
        QtCore.QObject.connect(self.ui.simpleNotesPrintButton,QtCore.SIGNAL("clicked()"), self.printNotes)
        QtCore.QObject.connect(self.ui.detailedNotesPrintButton,QtCore.SIGNAL("clicked()"), self.printNotesV)
        QtCore.QObject.connect(self.ui.referralLettersPrintButton,QtCore.SIGNAL("clicked()"), self.printReferral)
        QtCore.QObject.connect(self.ui.standardLetterPushButton,QtCore.SIGNAL("clicked()"), self.printLetter)
        QtCore.QObject.connect(self.ui.recallpushButton,QtCore.SIGNAL("clicked()"), self.exportRecalls)
        QtCore.QObject.connect(self.ui.account2_pushButton,QtCore.SIGNAL("clicked()"), self.accountButton2Clicked)

        #menu
        QtCore.QObject.connect(self.ui.action_save_patient,QtCore.SIGNAL("triggered()"), self.save_patient_tofile)
        QtCore.QObject.connect(self.ui.action_Open_Patient,QtCore.SIGNAL("triggered()"), self.open_patient_fromfile)
        QtCore.QObject.connect(self.ui.actionChoose_Database,QtCore.SIGNAL("triggered()"), self.changeDB)
        QtCore.QObject.connect(self.ui.action_About,QtCore.SIGNAL("triggered()"), self.aboutOM)
        QtCore.QObject.connect(self.ui.action_About_QT,QtCore.SIGNAL("triggered()"), QtGui.qApp, QtCore.SLOT("aboutQt()"))
        QtCore.QObject.connect(self.ui.action_Quit,QtCore.SIGNAL("triggered()"), self.quit)
        QtCore.QObject.connect(self.ui.actionTable_View_For_Charting,QtCore.SIGNAL("triggered()"), self.showChartTable)
        QtCore.QObject.connect(self.ui.actionClear_Today_s_Emergency_Slots,QtCore.SIGNAL("triggered()"), self.clearTodaysEmergencyTime)
        QtCore.QObject.connect(self.ui.actionTest_Print_an_NHS_Form,QtCore.SIGNAL("triggered()"), self.testGP17)
        QtCore.QObject.connect(self.ui.actionOptions,QtCore.SIGNAL("triggered()"), self.userOptionsDialog)

        #course ManageMent
        QtCore.QObject.connect(self.ui.newCourse_pushButton,QtCore.SIGNAL("clicked()"), self.newCourseSetup)
        ...
        #daybook - cashbook
        QtCore.QObject.connect(self.ui.daybookGoPushButton,QtCore.SIGNAL("clicked()"), self.daybookTab)
        QtCore.QObject.connect(self.ui.cashbookGoPushButton,QtCore.SIGNAL("clicked()"), self.cashbookTab)
        QtCore.QObject.connect(self.ui.daybookEndDateEdit,QtCore.SIGNAL("dateChanged ( const QDate & )"), self.datemanage)
        QtCore.QObject.connect(self.ui.daybookStartDateEdit,QtCore.SIGNAL("dateChanged ( const QDate & )"), self.datemanage)
        QtCore.QObject.connect(self.ui.cashbookEndDateEdit,QtCore.SIGNAL("dateChanged ( const QDate & )"), self.datemanage)
        QtCore.QObject.connect(self.ui.cashbookStartDateEdit,QtCore.SIGNAL("dateChanged ( const QDate & )"), self.datemanage)
        QtCore.QObject.connect(self.ui.cashbookPrintButton,QtCore.SIGNAL("clicked()"), self.cashbookPrint)
        QtCore.QObject.connect(self.ui.daybookPrintButton,QtCore.SIGNAL("clicked()"), self.daybookPrint)
        #accounts
        QtCore.QObject.connect(self.ui.loadAccountsTable_pushButton,QtCore.SIGNAL("clicked()"), self.populateAccountsTable)
        ...
        QtCore.QObject.connect(self.ui.chartsview_pushButton,QtCore.SIGNAL("clicked()"), self.showChartCharts)
        QtCore.QObject.connect(self.ui.summaryChartWidget,QtCore.SIGNAL("showHistory"), self.toothHistory)
        QtCore.QObject.connect(self.ui.staticChartWidget,QtCore.SIGNAL("showHistory"), self.toothHistory)

        QtCore.QObject.connect(self.ui.staticChartWidget,QtCore.SIGNAL("toothSelected"), self.static_chartNavigation)
        QtCore.QObject.connect(self.ui.planChartWidget,QtCore.SIGNAL("toothSelected"), self.plan_chartNavigation)
        QtCore.QObject.connect(self.ui.completedChartWidget,QtCore.SIGNAL("toothSelected"), self.comp_chartNavigation)

        QtCore.QObject.connect(self.ui.planChartWidget,QtCore.SIGNAL("completeTreatment"), self.completeToothTreatments)

        QtCore.QObject.connect(self.ui.toothPropsWidget,QtCore.SIGNAL("NextTooth"), self.navigateCharts)
        #--fillings have changed!!
        QtCore.QObject.connect(self.ui.toothPropsWidget,QtCore.SIGNAL("Changed_Properties"), self.updateCharts)  
        ...
        QtCore.QObject.connect(self.ui.toothPropsWidget,QtCore.SIGNAL("completed"), self.editCompleted)
        QtCore.QObject.connect(self.ui.toothPropsWidget,QtCore.SIGNAL("FlipDeciduousState"), self.flipDeciduous)

        #edit page
        QtCore.QObject.connect(self.ui.editMore_pushButton,QtCore.SIGNAL("clicked()"), self.showAdditionalFields)
        QtCore.QObject.connect(self.ui.defaultNP_pushButton,QtCore.SIGNAL("clicked()"), self.defaultNP)

        #notes page
        QtCore.QObject.connect(self.ui.notesMaximumVerbosity_radioButton,QtCore.SIGNAL("clicked()"), self.updateNotesPage)
        QtCore.QObject.connect(self.ui.notesMinimumVerbosity_radioButton,QtCore.SIGNAL("clicked()"), self.updateNotesPage)
        QtCore.QObject.connect(self.ui.notesMediumVerbosity_radioButton,QtCore.SIGNAL("clicked()"), self.updateNotesPage)

        #periochart
        #### defunct  QtCore.QObject.connect(self.ui.perioChartWidget,QtCore.SIGNAL("toothSelected"), self.periocharts)
        QtCore.QObject.connect(self.ui.perioChartDateComboBox,QtCore.SIGNAL("currentIndexChanged(int)"), self.layoutPerioCharts)
        QtCore.QObject.connect(self.ui.bpeDateComboBox,QtCore.SIGNAL("currentIndexChanged(int)"), self.bpe_table)

        #tab widget
        QtCore.QObject.connect(self.ui.main_tabWidget,QtCore.SIGNAL("currentChanged(int)"), self.handle_mainTab)
        QtCore.QObject.connect(self.ui.tabWidget,QtCore.SIGNAL("currentChanged(int)"), self.handle_patientTab)

        #main appointment tab
        QtCore.QObject.connect(self.ui.appointmentCalendarWidget,QtCore.SIGNAL("selectionChanged()"), self.layout_appointments)
        QtCore.QObject.connect(self.ui.goTodayPushButton,QtCore.SIGNAL("clicked()"), self.gotoToday)
        QtCore.QObject.connect(self.ui.printBook1_pushButton,QtCore.SIGNAL("clicked()"), self.book1print)
        QtCore.QObject.connect(self.ui.printBook2_pushButton,QtCore.SIGNAL("clicked()"), self.book2print)
        QtCore.QObject.connect(self.ui.printBook3_pushButton,QtCore.SIGNAL("clicked()"), self.book3print)
        QtCore.QObject.connect(self.ui.apptPrevDay_pushButton,QtCore.SIGNAL("clicked()"), self.apt_dayBack)
        QtCore.QObject.connect(self.ui.apptNextDay_pushButton,QtCore.SIGNAL("clicked()"), self.apt_dayForward)
        QtCore.QObject.connect(self.ui.apptPrevWeek_pushButton,QtCore.SIGNAL("clicked()"), self.apt_weekBack)
        QtCore.QObject.connect(self.ui.apptNextWeek_pushButton,QtCore.SIGNAL("clicked()"), self.apt_weekForward)
        QtCore.QObject.connect(self.ui.apptPrevMonth_pushButton,QtCore.SIGNAL("clicked()"), self.apt_monthBack)
        QtCore.QObject.connect(self.ui.apptNextMonth_pushButton,QtCore.SIGNAL("clicked()"), self.apt_monthForward)
        ...
        QtCore.QObject.connect(self.ui.aptOVnextweek,QtCore.SIGNAL("clicked()"), self.aptOV_weekForward)
        QtCore.QObject.connect(self.ui.aptOVprevmonth,QtCore.SIGNAL("clicked()"), self.aptOV_monthBack)
        QtCore.QObject.connect(self.ui.aptOVnextmonth,QtCore.SIGNAL("clicked()"), self.aptOV_monthForward)
        QtCore.QObject.connect(self.ui.aptOV_apptscheckBox,QtCore.SIGNAL("stateChanged(int)"), self.layout_apptOV)
        QtCore.QObject.connect(self.ui.aptOV_emergencycheckBox,QtCore.SIGNAL("stateChanged(int)"), self.layout_apptOV)
        QtCore.QObject.connect(self.ui.aptOV_lunchcheckBox,QtCore.SIGNAL("stateChanged(int)"), self.layout_apptOV)
        QtCore.QObject.connect(self.ui.aptOV_alldentscheckBox,QtCore.SIGNAL("stateChanged(int)"), self.apptOVdents)
        QtCore.QObject.connect(self.ui.aptOV_allhygscheckBox,QtCore.SIGNAL("stateChanged(int)"), self.apptOVhygs)
        QtCore.QObject.connect(self.ui.apptOVtoday_pushButton,QtCore.SIGNAL("clicked()"), self.gotoCurWeek)
        ...
        self.ui.tuesdayLabel.connect(self.ui.tuesdayLabel,QtCore.SIGNAL("clicked()"), self.tuesdaylabelClicked)
        self.ui.wednesdayLabel.connect(self.ui.wednesdayLabel,QtCore.SIGNAL("clicked()"), self.wednesdaylabelClicked)
        self.ui.thursdayLabel.connect(self.ui.thursdayLabel,QtCore.SIGNAL("clicked()"), self.thursdaylabelClicked)
        self.ui.fridayLabel.connect(self.ui.fridayLabel,QtCore.SIGNAL("clicked()"), self.fridaylabelClicked)

        #appointment manage
        QtCore.QObject.connect(self.ui.printDaylists_pushButton,QtCore.SIGNAL("clicked()"), self.daylistPrintWizard)
...
class customWidgets():
    def addCustomWidgets(self):
        ...
        #layout_appointments()            #commented out 25/4/2009. no need to lay out appointments until the tab is visited.
...
class cashbooks():    
    def cashbookTab(self):
        ...
        self.ui.cashbookTextBrowser.setHtml('<html><body><table border="1">'+html+"</table></body></html>")
    ...
    def daybookTab(self):
        ...
        self.ui.daybookTextBrowser.setHtml('<html><body><table border="1">'+html+"</table></body></html>")
        ...
        for row in range(self.ui.accounts_tableWidget.rowCount()):
            for col in range(12,15):
                ...
                if item.checkState():
                    ...
                    doc=accountPrint.document(printself.pt.title,printself.pt.fname,printself.pt.sname,
                    (printself.pt.addr1,printself.pt.addr2,printself.pt.addr3,printself.pt.town,printself.pt.county),printself.pt.pcde,localsettings.formatMoney(printself.pt.fees))
                    doc.setTone(tone)
                    if tone=="B":
...
class newPatientClass():
    ...
    def checkNewPatient(self):
        ...
        for widg in (self.ui.snameEdit,self.ui.titleEdit,self.ui.fnameEdit,self.ui.addr1Edit,self.ui.pcdeEdit):
            if len(widg.text())==0:
        ...
        if allfields_entered:
            ...
            self.advise("insufficient data to create a new record... please fill in all highlighted fields",2)
    ...
    def finishedNewPatientInput(self):
        ...
        QtCore.QObject.disconnect(self.ui.saveButton,QtCore.SIGNAL("clicked()"),self.checkNewPatient)                   
        QtCore.QObject.connect(self.ui.saveButton,QtCore.SIGNAL("clicked()"),self.save_changes)                
...
class printingClass():    
    ...
    def printReceipt(self,valDict,duplicate=False,dupdate=""):
        ...
        myreceipt.setProps(self.pt.title,self.pt.fname,self.pt.sname,self.pt.addr1,self.pt.addr2,self.pt.addr3,self.pt.town,self.pt.county,self.pt.pcde)
    ...
    def printApptCard(self):
        ...
        for row in range(rowcount):
            ...
            if localsettings.uk_to_sqlDate(adate)>=localsettings.sqlToday():
                futureAppts+=((adate,str(self.ui.ptAppointmentTableWidget.item(row,2).text()),str(self.ui.ptAppointmentTableWidget.item(row,1).text())),)
    ...
    def printaccount(self,tone="A"):
        if self.pt.serialno==0:
            ...
            (self.pt.addr1,self.pt.addr2,self.pt.addr3,self.pt.town,self.pt.county),self.pt.pcde,localsettings.formatMoney(self.pt.fees))
            doc.setTone(tone)
            if tone=="B":
...
class openmolarGui(customWidgets,newPatientClass,appointmentClass,signals,feeClass,printingClass,cashbooks):
    def __init__(self,parent):
        ...
        self.grid = ("ur8","ur7","ur6","ur5",'ur4','ur3','ur2','ur1','ul1','ul2','ul3','ul4','ul5','ul6',\
    ...
    def okToLeaveRecord(self):
        ...
        if uc != []:
            ...
            Dialog = QtGui.QDialog(self.mainWindow)
            dl = saveDiscardCancel.sdcDialog(Dialog,self.pt.fname+" "+self.pt.sname+" (%s)"%self.pt.serialno,uc)
            if Dialog.exec_():
    ...
    def updateCharts(self,arg):
        ...
        if self.selectedChartWidget=="st":
            ...
            self.advise("for the moment, please enter treatment into plan first, then complete it.",1)     
        else:
    ...
    def chartsTable(self):
        ...
        for tooth in self.grid:
            ...
            #-- I use this a lot. Every class has a  hidden __dict__ attribute to access attributes programatically self.pt.ur8st etc..
    ...
    def find_related(self):
        ...
        if candidates!=():
            ...
            dl.setupUi(Dialog)
            dl.selected=0
            dl.thisPatient_label.setText("Possible Matches for patient - %d - %s %s - %s"%(self.pt.serialno,self.pt.fname, self.pt.sname, self.pt.addr1))
    ...
    def loadpatient(self):
        ...
        briefHtml=estimates.toBriefHtml(self.pt.currEstimate)
        self.receptionSummary(briefHtml)
    ...
    def final_choice(self,candidates):
        ...
        QtCore.QObject.connect(dl.tableWidget,QtCore.SIGNAL("itemDoubleClicked (QTableWidgetItem *)"),DoubleClick)
            ...
            if result:
                ...
                if result[1] ==localsettings.ops[self.pt.dnt1]: 
                    ...
                    if self.pt.dnt2==0 or self.pt.dnt2==self.pt.dnt1:
                        ...
                        <br /><i>confirming this will remove reference to %s</i>'''%(result[1],localsettings.ops[self.pt.dnt2])
                        confirm=QtGui.QMessageBox.question(self.mainWindow,"Confirm",message,QtGui.QMessageBox.Yes, \
                        QtGui.QMessageBox.No)                                                                   
                    ...
                    confirm=QtGui.QMessageBox.question(self.mainWindow,"Confirm",message,QtGui.QMessageBox.Yes, QtGui.QMessageBox.No)                                                                   #check this was intentional!!
                    if confirm == QtGui.QMessageBox.Yes:                                                      
                        #--dialog rejected
    ...
    def unsavedChanges(self):
        ...
        if self.pt.serialno==self.pt_dbstate.serialno:
            ...
            self.advise( "POTENTIALLY SERIOUS CONFUSION PROBLEM WITH PT RECORDS %d and %d"%(self.pt.serialno,self.pt_dbstate.serialno),2)
            return changes

...
#localsettings.localsettings.humanTime,localsettings.minutesPastMidnight,localsettings.humanTime
...
class appointmentOverviewWidget(QtGui.QWidget):
        ...
        for dent in self.dents:
            ...
            for slot in self.freeslots[dent]:
                ...
                if rect.contains(event.pos()):
                    ...
                    QtGui.QToolTip.showText(event.globalPos(),QtCore.QString(feedback))
                    break
    ...
    def mousePressEvent(self, event):
        ...
        the signal has a LIST as argument - in case there are overlapping appointments or doubles etc...'''
        columnCount=len(self.dents)
        columnWidth=(self.width()-self.timeOffset)/columnCount
            ...
            if event.button()==1: #left click
                ...
                for slot in self.freeslots[dent]:
                    ...
                    if rect.contains(event.pos()):
                        self.highlightedRect=rect
                        feedback='%d mins starting at '%length +localsettings.wystimeToHumanTime(slotstart)+"  with "+localsettings.apptix_reverse[dent]
                        QtGui.QMessageBox.information(self,"Info","You've right clicked on a slot<br />%s"%feedback)
                        return
                ...
                for appt in self.appts[dent]:
                    ...
                    if rect.contains(event.pos()):
                        self.highlightedRect=rect
                        feedback='%d mins starting at '%length +localsettings.wystimeToHumanTime(slotstart)+"  with "+localsettings.apptix_reverse[dent]
                        QtGui.QMessageBox.information(self,"Info","You've right clicked on an appt<br />%s"%feedback)
                        return
                ...
                if self.lunch!=():
                    ...
                    if rect.contains(event.pos()):
                        self.highlightedRect=rect
                        feedback='%d mins starting at '%length +localsettings.wystimeToHumanTime(slotstart)+"  with "+localsettings.apptix_reverse[dent]
                        QtGui.QMessageBox.information(self,"Info","You've right clicked Lunch<br />%s"%feedback)
                        return
    ...
    def paintEvent(self,event=None):
        ...
        if True: #try:
            ...
            self.slotHeight=(self.height()-self.headingHeight)/self.slotCount
            columnCount=len(self.dents)
            if columnCount==0: columnCount=1 #avoid division by zero!!
            columnWidth=(self.width()-self.timeOffset)/columnCount
            while currentSlot<self.slotCount: #draw the grid
                ...
                if currentSlot%self.textDetail==0:
                    trect=QtCore.QRect(0,0.8*self.headingHeight+currentSlot*self.slotHeight,self.timeOffset,self.textDetail*self.slotHeight)
                    #painter.drawRect(trect) #remove this
                    painter.setPen(QtGui.QPen(QtCore.Qt.black,1))
                    painter.drawText(trect,QtCore.Qt.AlignHCenter,(QtCore.QString(localsettings.humanTime(self.startTime+(currentSlot*self.slotLength)))))
                currentSlot+=1
                col=0
            for dent in self.dents:
                leftx=self.timeOffset+(col)*columnWidth
